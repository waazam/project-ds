...
public partial class StalkerBody : Node3D
{
	...
	{
		// Neck: out of the front of the hump, forward and down.
		Vector3 hc = new(0.02f, 1.9f, 0.44f);   // head centre
		Loft(k, new Ring[]
		{
			new(NeckBase + new Vector3(0, 0.02f, -0.05f), 0.07f),
			new(NeckBase.Lerp(hc, 0.5f) + new Vector3(0, 0.01f, 0), 0.05f),
			new(hc + new Vector3(0, -0.02f, -0.06f), 0.045f),
		}, 6, false, false, side: Vector3.Right);

		// Head: small for the body, elongated, tilted, chin tucked. Built along its
		// own axis then posed; the front is a faint pale, featureless plane.
		...
		Loft(k, new Ring[]
		{
			new(new Vector3(0, -0.15f, 0.03f), 0.025f, 0.03f),   // narrow chin
			new(new Vector3(0, -0.11f, 0.02f), 0.05f, 0.065f),
			new(new Vector3(0, -0.04f, 0.0f), 0.068f, 0.09f),
			new(new Vector3(0, 0.03f, -0.01f), 0.075f, 0.1f),
			new(new Vector3(0, 0.09f, -0.03f), 0.07f, 0.095f),
			new(new Vector3(0, 0.14f, -0.05f), 0.05f, 0.075f),   // long cranium, swept back
			new(new Vector3(0, 0.17f, -0.07f), 0.022f, 0.035f),
		}, 10, true, true, tone: n =>
		{
		...
		// Two sockets on the face plane. Alpha 0 marks them as eyes: the skin shader
		// gives them a faint yellow glow (every other vertex has alpha 1).
		k.Color = new Color(0f, 0.3f, 0f, 0f);
		foreach (float ex in new[] { -0.028f, 0.028f })
		{
			Vector3 c = new(ex, 0.025f, 0.09f);
